...
//https://stackoverflow.com/questions/21332040/simple-cuda-kernel-optimization/21340927#21340927
    ...
    __global__ void substractArrays(float* vec,float* vec2,const size_t n)
    {
        ...
        __device__ __inline__
            float gradient_HOTV(const float* u,
            long z, long y, long x,
            long depth, long rows, long cols){
        unsigned long size2d = rows*cols;
        unsigned long long idx = z * size2d + y * cols + x;
        if ( z - 2 >= 0 && z < depth - 2 && y - 2 >= 0 && y< rows - 2 && x - 2 >= 0 && x < cols - 2) {
		float eps=0.00000001; //% avoid division by zero

		long a = x;long b = y;long c = z;
		float uidx = u[z*size2d + b*cols + a];
		float s1_t = u[z*size2d + b*cols + (a-1)];		
		float s_t1 = u[z*size2d + (b-1)*cols + a];		
		float s1_t1 = u[z*size2d + (b-1)*cols + (a-1)];
		float s2_t = u[z*size2d + b*cols + (a-2)];		
		float s_t2 = u[z*size2d + (b-2)*cols + a];		
		float Denom1 =  sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer1 = 4 * uidx - 4 * s1_t - 4 * s_t1 + 2 * s1_t1 + s2_t + s_t2;

		a = x + 1;b = y;
		uidx = u[z*size2d + b*cols + a];
		s1_t = u[z*size2d + b*cols + (a-1)];		
		s_t1 = u[z*size2d + (b-1)*cols + a];		
		s1_t1 = u[z*size2d + (b-1)*cols + (a-1)];
		s2_t = u[z*size2d + b*cols + (a-2)];		
		s_t2 = u[z*size2d + (b-2)*cols + a];		
		float Denom2 = sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer2 = 6 * s1_t - 4 * uidx - 2 * s2_t - 2 * s1_t1 + 2 * s_t1;
		...
		s2_t = u[z*size2d + b*cols + (a-2)];		
		s_t2 = u[z*size2d + (b-2)*cols + a];
		float Denom3 = sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer3 = 6 * s_t1 - 4 * uidx - 2 * s1_t1 - 2 * s_t2 + 2 * s1_t;
		...
		s2_t = u[z*size2d + b*cols + (a-2)];		
		s_t2 = u[z*size2d + (b-2)*cols + a];	
		float Denom4 = sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer4 = uidx - 2 * s1_t +  s2_t;
		...
		s2_t = u[z*size2d + b*cols + (a-2)];		
		s_t2 = u[z*size2d + (b-2)*cols + a];	
		float Denom5 = sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer5 = uidx - 2 * s_t1 +  s_t2;
		...
		s2_t = u[z*size2d + b*cols + (a-2)];		
		s_t2 = u[z*size2d + (b-2)*cols + a];
		float Denom6 = sqrt( eps + pow((uidx - 2*s1_t + s2_t),2) + 2*pow((uidx - s1_t - s_t1 + s1_t1),2) + pow((uidx- 2*s_t1 + s_t2),2) );
		float Numer6 = 2* uidx + 2 * s1_t1 - 2 * s_t1 - 2 * s1_t;

		return (Numer1/Denom1 + Numer2/Denom2 + Numer3/Denom3 + Numer4/Denom4 + Numer5/Denom5 + Numer6/Denom6);
        }
    ...
    __global__ void gradientTV(const float* f, float* dftv,
            long depth, long rows, long cols, bool HOTV, bool RwTV, float delta){
        ...
        if (HOTV == false){
			...
			if(RwTV == false){
				...
				if(dfj[0] == u || dfj[0] > u)	dfj[0]=u;
				if(dfj[1] == u || dfj[1] > u)	dfj[1]=u;
				if(dfj[2] == u || dfj[2] > u)	dfj[2]=u;
				if(dfk[0] == u || dfk[0] > u)	dfk[0]=u;
				if(dfk[1] == u || dfk[1] > u)	dfk[1]=u;
				if(dfk[2] == u || dfk[2] > u)	dfk[2]=u;
				...
				//2-D TV
				/* dftv[idx]=(df[1]+df[2])/(sqrt(df[1] *df[1] +df[2] *df[2])+eps)
				-dfi[2]/(sqrt(dfi[1]*dfi[1]+dfi[2]*dfi[2]) +eps)     // I wish I coudl precompute this, but if I do then Id need to recompute the gradient.
				-dfj[1]/(sqrt(dfj[1]*dfj[1]+dfj[2]*dfj[2]) +eps);*/
			}
			else{
				//exp type
				...
				if(wy == -u || wy < -u)	wy=-u;
				if(wz == -u || wz < -u)	wz=-u;
				if(wxi == -u || wxi < -u)	wxi=-u;
				if(wyi == -u || wyi < -u)	wyi=-u;
				if(wzi == -u || wzi < -u)	wzi=-u;
				if(wxj == -u || wxj < -u)	wxj=-u;
				if(wxk == -u || wxk < -u)	wxk=-u;
				if(wyk == -u || wyk < -u)	wyk=-u;
				if(wzk == -u || wzk < -u)	wzk=-u;*/

				//Reciprocal type
				/*float eps1 = 0.0000000000000000001;
				float wx=1/(abs(df[0])+eps1);
				float wy=1/(abs(df[1])+eps1);
				float wz=1/(abs(df[2])+eps1);
        
				float wxi=1/(abs(dfi[0])+eps1);
				float wyi=1/(abs(dfi[1])+eps1);
				float wzi=1/(abs(dfi[2])+eps1);
        
				float wxj=1/(abs(dfj[0])+eps1);
				float wyj=1/(abs(dfj[1])+eps1);
				float wzj=1/(abs(dfj[2])+eps1);
        
				float wxk=1/(abs(dfk[0])+eps1);
				float wyk=1/(abs(dfk[1])+eps1);
				...
				float wx=(cos(pi*df[0]/u)+1)/2;
				float wy=(cos(pi*df[1]/u)+1)/2;
				float wz=(cos(pi*df[2]/u)+1)/2;
        
				float wxi=(cos(pi*dfi[0]/u)+1)/2;
				float wyi=(cos(pi*dfi[1]/u)+1)/2;
				float wzi=(cos(pi*dfi[2]/u)+1)/2;
        
				float wxj=(cos(pi*dfj[0]/u)+1)/2;
				float wyj=(cos(pi*dfj[1]/u)+1)/2;
				float wzj=(cos(pi*dfj[2]/u)+1)/2;
				...
				/*dftv[idx]=(wy*df[1]+wz*df[2])/(sqrt(wy*df[1] *df[1] +wz*df[2] *df[2])+eps)
				-wzi*dfi[2]/(sqrt(wyi*dfi[1]*dfi[1]+wzi*dfi[2]*dfi[2]) +eps)     // I wish I coudl precompute this, but if I do then Id need to recompute the gradient.
				-wyj*dfj[1]/(sqrt(wyj*dfj[1]*dfj[1]+wzj*dfj[2]*dfj[2]) +eps);*/
			...
			dftv[idx] = 0;
		    dftv[idx] = gradient_HOTV(f, z, y, x, depth, rows, cols);
		}
...
// main function
    void pocs_tv(float* img,float* dst,float alpha,const long* image_size, int maxIter, bool HOTV, bool RwTV, float delta){
            ...
            for(unsigned int sp=0;sp<splits;sp++){
                ...
                for(unsigned int ib=0;  (ib<(buffer_length-1)) && ((i+ib)<maxIter);  ib++){
                    ...
                    for (dev = 0; dev < deviceCount; dev++){
                        ...
                        gradientTV<<<gridGrad, blockGrad,0,stream[dev*nStream_device]>>>(d_image[dev],d_dimgTV[dev],(long)(curr_slices+buffer_length*2-1), image_size[1],image_size[0],HOTV,RwTV,delta);
